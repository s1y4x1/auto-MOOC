from 单元作业 import *
from 单元测试 import *
mocTerm=post(url_check,{'termId':1475054441})['result']['mocTermDto']
...
keys={
    ...
    'exam':['id','name','totalScore','userScore']
    }
chapters=[{'name':l['name'],
           ...
           'exam':{k:l['exam'][k]for k in keys['exam']}if l['exam']else None
           }for l in chapters]
...
for l in chapters:
    ...
    for h in l['homeworks']:
        p(h['name'])
        tab+=1
        p('已'if h['deadline']/1000<time.time()else'未','截止（',t(h['deadline']),'）')
        ...
        if h['deadline']/1000>=time.time()and has_empty(h['id']):
            p('[!]存在未提交，尝试操作……')
            p(*homework(h['id']))
        tab-=1
    for q in l['quizs']:
        p(q['name'])
        tab+=1
        p('已'if q['deadline']/1000<time.time()else'未','截止（',t(q['deadline']),'）')
        p('得分：',q['userScore'],'/',q['totalScore'])
        if not q['userScore']or q['userScore']<q['totalScore']:
            if q['deadline']/1000<time.time():p('[!]未满分但已截止，只得跳过。')
            else:
                p('[!]当前未满分，尝试操作……')
                p(*quiz(q['id']))
        tab-=1
